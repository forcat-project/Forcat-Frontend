...
export default function Cats() {
    ...
    const handleSave = (updatedCat: Cat) => {
        setCats(prevCats => prevCats.map(cat => (cat.cat_id === updatedCat.cat_id ? updatedCat : cat)));
        closeEditModal();
    };
    ...
    return (
        <Block.FlexBox padding="20px" direction="column">
            <Block.FlexBox justifyContent="space-between" alignItems="center" padding="0 20px">
                <Text.TitleMenu200 style={{ marginLeft: "-20px" }}>우리집 냐옹이들</Text.TitleMenu200>
                ...
                >
                    <Block.FlexBox direction="column">
                        ...
                        <Block.FlexBox margin="5px 0">
                            ...
                            <Text.Notice200>{String(cat.cat_breed_name) || "정보 없음"}</Text.Notice200>
                        </Block.FlexBox>
                        <Block.FlexBox margin="5px 0">
                            <Text.Notice200 color="Gray" style={{ marginRight: "10px" }}>
                                나이
                            </Text.Notice200>
                            <Text.Notice200>{getMonthsFromDays(cat.days_since_birth || 0)}</Text.Notice200>
                        </Block.FlexBox>
                        <Block.FlexBox margin="5px 0">
                            <Text.Notice200 color="Gray" style={{ marginRight: "10px" }}>
                                성별
                            </Text.Notice200>
                            <Text.Notice200>{cat.gender === 0 ? "여아" : "남아"}</Text.Notice200>
                        </Block.FlexBox>
                        <Block.FlexBox margin="5px 0">
                            <Text.Notice200 color="Gray" style={{ marginRight: "10px" }}>
                                중성화
                            </Text.Notice200>
                            <Text.Notice200>{cat.is_neutered === 1 ? "수술 완료" : "수술 미완료"}</Text.Notice200>
                        </Block.FlexBox>
                        <Block.FlexBox margin="5px 0">
                            <Text.Notice200 color="Gray" style={{ marginRight: "10px" }}>
                                몸무게
                    ...
                    <Block.FlexBox alignItems="center" width="150px" height="150px" direction="column">
                        {cat.profile_image ? (
                            <img
                        ...
                        <Button.EditButton style={{ marginTop: "15px" }} onClick={() => openEditModal(cat)}>
            ...
            >
                {selectedCat && (
                    <CatEdit cat={selectedCat} onClose={closeEditModal} onReload={fetchCats} onSave={handleSave} />
                )}
            </ReactModal>
        </Block.FlexBox>
    );
