...
export default function OnlySearch() {
    ...
    const fetchProducts = async (searchValue: string, cursorValue: string | null = null) => {
        ...
        try {
            ...
            setProducts(prevProducts => [...prevProducts, ...response.data.results]); // 이전 결과에 새로운 결과 추가
            const nextCursor = response.data.next ? new URL(response.data.next).searchParams.get("cursor") : null;
